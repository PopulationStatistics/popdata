...
The `popdata` package requires you to add your `username`, `password` and the session `cookie`.



You can store your username and password in your `.Renviron` file or access it by calling `usethis::edit_r_environ()` (assuming you have the `usethis` package installed) and entering:
...
The third environment variable used is `POPDATA_COOKIE` where you can temporarily store the value of the session cookie.

Once the environment variables set you will need to restart your session.
...
If you stored your `username` and `password` in the .Renviron under `POPDATA_USER` and `POPDATA_PWD`, you just need to set your session cookie.

```{r, eval = FALSE}
popdata_setup(cookie = "xxxxxxxxxxxx")
```

Now that we are connected to POPDATA, we can download data from the Annual Statistical report.
